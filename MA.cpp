...
void two_opt_search(vector<int>& tour, const vector<City>& cities) {
    bool improvement = true;
    while (improvement) {
        improvement = false;
        for (size_t i = 0; i < tour.size() - 1; ++i) {
            for (size_t j = i + 2; j < tour.size(); ++j) {
                double current_distance = calculate_distance(cities[tour[i]], cities[tour[i + 1]]) + calculate_distance(cities[tour[j]], cities[tour[(j + 1) % tour.size()]]);
                double new_distance = calculate_distance(cities[tour[i]], cities[tour[j]]) + calculate_distance(cities[tour[i + 1]], cities[tour[(j + 1) % tour.size()]]);
...
vector<vector<int>> generate_offspring(const vector<vector<int>>& population, const vector<double>& fitnesses, const vector<City>& cities) {
    ...
    for (int i = 0; i < population_size; ++i) {
        ...
        two_opt_search(child, cities); // Apply local search
...
int main() {
    string filename = "C:/dev/RU/CPM_collective_migration/Assigment_5/bayg29.tsp";
